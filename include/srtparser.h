...
private:
  ...
  bool _ignore; // should subtitle be ignore; used when the subtitle is empty
                // after processing
  ...
  std::vector<std::string>
      _nonDialogue;               // list of non dialogue words, e.g. (applause)
  ...
  std::vector<long int>
      _wordDuration;  // actual duration of each word without silence
  ...
  void extractInfo(bool keepHTML = 0, bool doNotIgnoreNonDialogues = 0,
                   bool doNotRemoveSpeakerNames = 0); // process subtitle
public:
  ...
  std::string
  getStartTimeString() const; // returns sarting time as present in .srt file
  std::string
  getEndTimeString() const;     // returns ending time as present in .srt file
  bool getIgnoreStatus() const; // returns status, whether the subtitle is
                                // ignorable or not after processing
  std::string
  getDialogue(bool keepHTML = 0, bool doNotIgnoreNonDialogues = 0,
              bool doNotRemoveSpeakerNames = 0); // returns processed subtitle
  ...
  std::vector<std::string>
  getIndividualWords(); // return string vector of individual words
  std::string getWordByIndex(int index);     // return word stored at 'index'
  std::vector<long int> getWordStartTimes(); // return long int vector of start
                                             // time of individual words
  std::vector<long int>
  getWordEndTimes(); // return long int vector of end time of individual words
  long int getWordStartTimeByIndex(
      int index); // return the start time of a word based on index
  long int getWordEndTimeByIndex(
      int index); // return the end time of a word based on index
  std::vector<std::string>
  getSpeakerNames(); // return string vector of speaker names
  std::vector<std::string>
  getNonDialogueWords(); // return string vector of non dialogue words
  ...
  void setWordTimes(
      std::vector<long int> wordStartTime, std::vector<long int> wordEndTime,
      std::vector<long int> wordDuration); // assign time to individual words

  SubtitleItem(void);
  SubtitleItem(
      int subNo, std::string startTime, std::string endTime, std::string text,
      bool ignore = false, std::string justDialogue = "", int speakerCount = 0,
      int nonDialogueCount = 0, int styleTagCount = 0, int wordCount = 0,
      ...
      std::vector<std::string> word =
          std::vector<std::string>()); // default constructor
  ~SubtitleItem(void);
};
...
inline void SubRipParser::parse(std::string fileName) // srt parser
{
  ...
  if (3 == turn) // insert last remaining subtitle
  {
    _subtitles.push_back(new SubtitleItem(subNo, start, end, completeLine));
  }
}
...
inline SubtitleItem::SubtitleItem(
    int subNo, std::string startTime, std::string endTime, std::string text,
    bool ignore, std::string justDialogue, int speakerCount,
    int nonDialogueCount, int styleTagCount, int wordCount,
...
inline long int SubtitleItem::getStartTime() const { return _startTime; }
inline long int SubtitleItem::getEndTime() const { return _endTime; }

inline std::string SubtitleItem::getText() const { return _text; }

inline void SubtitleItem::setStartTime(long int startTime) {
  _startTime = startTime;
}
inline void SubtitleItem::setEndTime(long int endTime) { _endTime = endTime; }
inline void SubtitleItem::setText(std::string text) { _text = text; }
inline void SubtitleItem::setWordTimes(std::vector<long int> wordStartTime,
                                       std::vector<long int> wordEndTime,
                                       std::vector<long int> wordDuration) {
  _wordStartTime = wordStartTime;
  _wordEndTime = wordEndTime;
  _wordDuration = wordDuration;
}
inline int SubtitleItem::getSubNo() const { return _subNo; }
...
inline void
SubtitleItem::extractInfo(bool keepHTML, bool doNotIgnoreNonDialogues,
                          bool doNotRemoveSpeakerNames) // process subtitle
{
  ...
  if (!doNotRemoveSpeakerNames) {
    for (int i = 0; output[i] != '\0'; i++) {
      ...
      if (output[i] == ':') // speaker found; travel back
      {
        ...
        int tempIndex = 0, foundEvilColon = 0, continueFlag = 0,
            spaceBeforeColon = 0;
        ...
        Elon Musk: Hey Saurabh, you are pretty smart.       // First and Last
        Name Saurabh: *_* What? Elon Musk: Yes!                  // Two names in
        single line Saurabh : OMG OMG!                                  // Space
        before colon Elon: LOL World: LAMAO Saurabh: ._. // normal
          ...
          if (output[j] == '.' || output[j] == '!' || output[j] == ',' ||
            ...
            if (output[j] == '.' || output[j] == '!' || output[j] == ',' ||
              ...
              continue;
                     ...
                     !isupper(output[j + 1] && tempIndex == 0)) {
              _speakerCount--;
              foundEvilColon = 1;
              break;
            }
        ...
        _speaker.push_back(
            output.substr(nameBeginIndex, colonIndex - nameBeginIndex));
...
inline std::string SubtitleItem::getDialogue(bool keepHTML,
                                             bool doNotIgnoreNonDialogues,
                                             bool doNotRemoveSpeakerNames) {
  if (_justDialogue.empty())
    extractInfo(keepHTML, doNotIgnoreNonDialogues, doNotRemoveSpeakerNames);

  return _justDialogue;
}
inline int SubtitleItem::getSpeakerCount() const { return _speakerCount; }
inline int SubtitleItem::getNonDialogueCount() const {
  return _nonDialogueCount;
}
inline int SubtitleItem::getStyleTagCount() const { return _styleTagCount; }
inline int SubtitleItem::getWordCount() const { return _wordCount; }
...
inline SubtitleWord::SubtitleWord(void) { _text = ""; }

inline SubtitleWord::SubtitleWord(std::string text) { _text = text; }

inline std::string SubtitleWord::getText() const { return _text; }
