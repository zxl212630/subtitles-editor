...
int main(int argc, char *argv[]) {
  ...
  if (runBenchmark) {
    ...
    qInfo() << "[BENCHMARK] Starting video preview benchmark on file:" << benchmarkVideoPath;
    ...
    runner->scenarios = {
      ...
      { "Fast Forward Drag (10ms interval, 100ms step, highly sequential)", 10, 100 },
      { "Backward Drag (25ms interval, -200ms step, sequential-like)", 25, 50 },
      { "Large Random Jumps (200ms interval, non-sequential timeline clicks)", 200, 20 },
    ...
    QObject::connect(&player, &MediaPlayer::previewFrameRendered, [runner](qint64 ptsMs, qint64 targetMs) {
      if (runner->currentScenarioIdx >= runner->scenarios.size()) return;
      auto &s = runner->stats[runner->currentScenarioIdx];
      s.framesRendered++;
      qint64 error = qAbs(ptsMs - targetMs);
      s.totalErrorMs += error;
      s.errorCount++;
      qInfo() << QString("[%1] Rendered preview frame at %2 ms for target %3 ms, error: %4 ms")
                 .arg(runner->scenarios[runner->currentScenarioIdx].name)
                 .arg(ptsMs).arg(targetMs).arg(error);
    });
    
    QObject::connect(&player, &MediaPlayer::mediaLoaded, [&player, runner](qint64 durationMs, QSize size) {
      qInfo() << "[BENCHMARK] Media loaded successfully. Duration:" << durationMs << "ms, Size:" << size.width() << "x" << size.height();
      ...
      auto runNextScenario = [&player, runner]() {
        if (runner->currentScenarioIdx >= runner->scenarios.size()) {
          ...
          qInfo() << "[BENCHMARK] All scenarios complete. Stopping preview dragging.";
          player.stopPreviewDragging();
          
          // Wait 500ms for final precise seek to complete, then print report
          QTimer::singleShot(500, [runner]() {
            ...
            for (int i = 0; i < runner->scenarios.size(); ++i) {
              ...
              qInfo() << QString("Scenario %1: %2").arg(i + 1).arg(scen.name);
              qInfo() << QString("  - Seeks Requested: %1").arg(st.seekRequests);
              qInfo() << QString("  - Frames Rendered: %1").arg(st.framesRendered);
              if (st.seekRequests > 0) {
                double renderRate = (st.framesRendered * 100.0) / st.seekRequests;
                qInfo() << QString("  - Render/Coalesce Success Rate: %1%").arg(renderRate, 0, 'f', 1);
              }
              if (st.errorCount > 0) {
                qInfo() << QString("  - Average Timestamp Error: %1 ms").arg(st.totalErrorMs / st.errorCount, 0, 'f', 2);
              } else {
        ...
        qInfo() << QString("\n>>> Starting Scenario %1: %2").arg(runner->currentScenarioIdx + 1).arg(scen.name);
      ...
      QObject::connect(runner->dragTimer, &QTimer::timeout, [&player, runner, runNextScenario]() {
        if (runner->currentScenarioIdx >= runner->scenarios.size()) return;
        const auto &scen = runner->scenarios[runner->currentScenarioIdx];
        auto &st = runner->stats[runner->currentScenarioIdx];
        
        if (runner->currentStep >= scen.totalSteps) {
          st.endTime = QDateTime::currentMSecsSinceEpoch();
          runner->dragTimer->stop();
          qInfo() << QString(">>> Scenario %1 finished.").arg(runner->currentScenarioIdx + 1);
          runner->currentScenarioIdx++;
          // Wait 100ms before starting next scenario to let current decoder cool down
        ...
        if (runner->currentScenarioIdx == 0) {
          ...
          targetMs = static_cast<qint64>(ratio * runner->durationMs * 0.9); // keep within 90% of duration
        } else if (runner->currentScenarioIdx == 4) {
          // Within GOP (Small span): jump forward by 500ms each step (0.5s is < 2s, sequential)
          targetMs = runner->currentStep * 500;
        } else if (runner->currentScenarioIdx == 5) {
          // Within GOP (Medium span): jump forward by 4000ms each step (4s is > 2s so full seek, but in same/adjacent GOP)
          targetMs = runner->currentStep * 4000;
        } else if (runner->currentScenarioIdx == 6) {
          // Across Few GOPs: jump forward by 15000ms each step (15s = 1.5 GOPs)
          targetMs = runner->currentStep * 15000;
        } else if (runner->currentScenarioIdx == 7) {
          // Across Many GOPs: jump forward by 50000ms each step (50s = 5 GOPs)
    ...
    QObject::connect(&player, &MediaPlayer::playbackError, [](const QString &err) {
