...
void ThemeSelectorWidget::addTheme(const QString &id, const QString &bgBase, const QString &bgPanel, const QString &primary) {
...
QString ThemeSelectorWidget::currentTheme() const {
    return currentId_;
}
...
void ThemeSelectorWidget::paintEvent(QPaintEvent *) {
    ...
    for (int i = 0; i < items_.size(); ++i) {
        ...
        p.fillRect(sidebarRect.x() + 4, sidebarRect.y() + 8, 12, 2, QColor("#888888"));
        p.fillRect(sidebarRect.x() + 4, sidebarRect.y() + 14, 8, 2, QColor("#888888"));
        p.fillRect(sidebarRect.x() + 4, sidebarRect.y() + 20, 10, 2, QColor("#888888"));

        // Content mock lines
        int contentX = sidebarRect.right() + 8;
        p.fillRect(contentX, rect.y() + 8, 20, 4, QColor(item.primary)); // Mock title
        p.fillRect(contentX, rect.y() + 18, 35, 3, QColor(item.bgPanel).lighter(120)); 
        p.fillRect(contentX, rect.y() + 26, 25, 3, QColor(item.bgPanel).lighter(120)); 
        p.fillRect(contentX, rect.y() + 34, 40, 3, QColor(item.bgPanel).lighter(120)); 
...
QString ColorSelectorWidget::currentColor() const {
    return currentId_;
}
...
void ColorSelectorWidget::paintEvent(QPaintEvent *) {
    ...
    for (int i = 0; i < items_.size(); ++i) {
        ...
        if (item.id == currentId_) {
            ...
            p.drawLine(center.x() - 1, center.y() + 4, center.x() + 5, center.y() - 4);
        } else {
            p.setPen(Qt::NoPen);
            p.setBrush(QColor(item.hex));
            p.drawEllipse(center, radius, radius);
